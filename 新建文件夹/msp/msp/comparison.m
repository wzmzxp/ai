...
for Algnum=[1];  %选取使用的测试算法(上表)
    ...
    namestr=['.\',Algstr{Algnum},['_result_D',num2str(D),'_P',num2str(pop_size),'_',datestr(now, 'yyyymmddHHMMSS')]];
    txtstr=[namestr,'.txt'];
    fp=fopen(txtstr,'w+');
    popCishu=popCishu+1;
    for i=1:funcnum   %选择测试函数，func.m中有14个高维函数，单独测试，写成funcnum:funcnum
        ...
        for j=1:runs %j为设置的循环次数
            if Algnum==1
                ...
                [gbest,gbestval,Convergence,FES]= mqhao_with_subgroup2(D,pop_size,group,gr,iter_max,Xmin(i),Xmax(i),func_num);
